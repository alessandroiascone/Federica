...
let wizard={step:0,studentIds:[],date:'',time:'',duration:60,mode:'presence',rateType:null,repeatWeeks:1,recoveryOf:null};
...
function migrateData(){
  ...
  data.meta.schema=Math.max(Number(data.meta.schema)||4,8);
...
function paymentBreakdownHtml(x){
  ...
  return `<div class="note price-note"><b>Conteggio automatico:</b><br>${lines}<br><small>La durata è informativa: anche 1,5 ore conta come una sola lezione al prezzo scelto.</small></div>`;
}
...
function wizardNextStudents(){
  ...
  if(wizard.studentIds.length===1&&!isMonthlyStudent(wizard.studentIds[0])){const st=studentById(wizard.studentIds[0]);wizard.rateType=st?.preferredRate||wizard.rateType||null}else if(wizard.studentIds.length>1){wizard.rateType='collective'}
  wizard.step=1;renderWizard();
}
function wizardNextDate(){if(!wizard.date){toast('Scegli un giorno');return}if(!ensureMonthOpen(wizard.date.slice(0,7),'aggiungere una lezione'))return;wizard.step=2;renderWizard()}
...
function analyzeWizardConflicts(){
  return wizardOccurrences().map(o=>{
    ...
    const mergeTarget=!sameStudent.length&&overlaps.length===1&&exact.length===1?exact[0]:null;
...
function mergeStudentsIntoLesson(target){
  ...
  if(billable.length){target.rateType=target.studentIds.length>1?'collective':(target.rateType||wizard.rateType);target.pricePerStudent=target.studentIds.length>1?10:(Number(target.pricePerStudent)||rateAmount(target.rateType));target.pricingMissing=false}
...
function commitWizardLessons(mergeExisting=false){
  ...
  closeModal();
  const total=created.length+merged;toast(total>1?`${total} lezioni registrate`:merged?'Alunno aggiunto alla lezione cumulativa':created[0]?.status==='completed'?'Lezione passata registrata':'Lezione aggiunta');
...
function moveStudentFromLesson(lid,sid){
  ...
  if(!confirm(`Modificare solo la lezione di ${st.name}? Verrà separata dalla cumulativa; gli altri alunni resteranno invariati.`))return;
  const copy=cloneLessonForStudent(l,sid,{detachedForEdit:true});removeStudentParticipationRaw(l,sid);data.lessons.push(copy);save('Separazione alunno da lezione cumulativa');openLessonEdit(copy.id);toast('Ora modifichi solo questo alunno');
}
...
function openThirtyDaySummary(){
  ...
  const all=days30.flatMap(x=>x.lessons),occupied=days30.filter(x=>x.lessons.length).length,appointments=all.reduce((n,l)=>n+l.studentIds.length,0);
  replaceModal(`<h2>Prossimi 30 giorni</h2><p class="sub">Dal ${fmtDate(start)} al ${fmtDate(end)}. Qui trovi anche le lezioni create in serie.</p><div class="statrow"><div class="stat"><b>${all.length}</b><small>Lezioni</small></div><div class="stat"><b>${appointments}</b><small>Posti alunno</small></div><div class="stat"><b>${occupied}</b><small>Giorni occupati</small></div></div><div class="note"><b>Gestione rapida:</b> tocca una lezione per modificarla, annullarla o eliminare anche un solo alunno senza toccare gli altri della cumulativa.</div><div class="range30">${days30.map((x,i)=>`<div class="range-day ${i===0?'today-range':''}"><button class="range-head" onclick="renderSummaryModal('${x.dk}',true)"><span class="range-date"><b>${x.d.getDate()}</b><small>${days[x.d.getDay()].slice(0,3)}</small></span><span class="range-title"><b>${fmtDate(x.dk)}</b><small>${x.lessons.length?`${x.lessons.length} ${x.lessons.length===1?'lezione':'lezioni'}`:'Nessuna lezione'}</small></span><span class="arrow">›</span></button>${x.lessons.length?`<div class="range-lessons">${x.lessons.map(l=>`<button class="range-lesson" onclick="openLesson('${l.id}')"><span class="range-time">${l.time}</span><span><b>${esc(lessonStudentNames(l))}</b><small>${l.mode==='dad'?'DAD':'Presenza'} · ${l.studentIds.length>1?'Cumulativa':'Singola'}${l.seriesId?' · Ricorrente':''}</small></span><span class="range-mode ${l.mode==='dad'?'dad':'presence'}">${l.mode==='dad'?'DAD':'Aula'}</span></button>`).join('')}</div>`:''}</div>`).join('')}</div>`);
}
function renderSummaryModal(k,push=false){
  const ls=data.lessons.filter(l=>l.date===k&&l.status!=='cancelled').sort((a,b)=>a.time.localeCompare(b.time));
  const ended=ls.filter(l=>l.status==='scheduled'&&lessonHasEnded(l)),dayRevenue=ls.filter(isAccountingLesson).reduce((a,l)=>a+lessonRevenue(l),0),missing=ls.filter(l=>isAccountingLesson(l)&&!hasValidPricing(l)).length;
...
function reconcileAllCompleted(){const p=pendingPastLessons();if(!p.length)return;if(p.some(l=>isMonthClosed(l.date.slice(0,7)))){toast('Ci sono lezioni in un mese chiuso: riaprilo prima');return}if(!confirm(`Confermare come svolte tutte le ${p.length} lezioni?`))return;p.forEach(l=>{l.status='completed';l.completedAt=new Date().toISOString();l.updatedAt=new Date().toISOString()});save('Verifica massiva lezioni');if(modalStack.length)modalBack();else closeModal();toast('Tutte le lezioni sono state conteggiate')}
...
function paymentInfo(sid,mk){
  ...
  const dadLs=completedLs.filter(l=>l.mode==='dad'),presenceLs=completedLs.filter(l=>l.mode==='presence'),dad=dadLs.reduce((a,l)=>a+l.duration/60,0),presence=presenceLs.reduce((a,l)=>a+l.duration/60,0);
  ...
  return {student:st,ls,completedLs,pending,missingPrice,dad,presence,dadLessons:dadLs.length,presenceLessons:presenceLs.length,completedCount:completedLs.length,absentBilledCount:absentBilled.length,hours:dad+presence,autoAmount,p,key,planType,monthlyAmount,monthlyActivityCount:monthlyActivity.length};
}
