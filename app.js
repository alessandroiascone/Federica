...
function defaultData(){
  return {
    ...
    payments:{},
    monthClosures:{},
    settings:{start:'09:00',end:'20:00',step:30},
...
function migrateData(){
  ...
  data.settings=Object.assign({start:'09:00',end:'20:00',step:30},data.settings||{});data.settings.start='09:00';data.settings.end='20:00';data.settings.step=30;delete data.settings.dadLink;
  ...
  data.students.forEach(s=>{
    ...
    if(!['individual','regular','collective'].includes(s.preferredRate))s.preferredRate='';
  });
...
function billingFormHtml(prefix,s={}){
  const type=studentBillingType(s),amount=esc(s.monthlyAmount??''),dur=Number(s.monthlyDuration)||60,pref=['individual','regular','collective'].includes(s.preferredRate)?s.preferredRate:'individual';
  return `<div class="field"><label>Tipo pagamento</label><select class="input" id="${prefix}Billing" onchange="toggleBillingFields('${prefix}')"><option value="lesson" ${type==='lesson'?'selected':''}>A lezione</option><option value="monthly" ${type==='monthly'?'selected':''}>Mensile</option></select></div><div id="${prefix}LessonBox" style="${type==='lesson'?'':'display:none'}"><div class="field"><label>Tariffa automatica</label><select class="input" id="${prefix}PreferredRate"><option value="individual" ${pref==='individual'?'selected':''}>Individuale · €20</option><option value="regular" ${pref==='regular'?'selected':''}>Cliente abituale · €15</option><option value="collective" ${pref==='collective'?'selected':''}>Collettiva · €10</option></select></div><div class="note"><b>Calcolo automatico:</b> puoi impostare la tariffa abituale dell’alunno a €20, €15 oppure Collettiva €10. Se è impostato Collettiva, il flusso proporrà automaticamente la lezione collettiva; potrai comunque cambiarla quel giorno. Il totale mensile resta sempre modificabile prima dell’invio.</div></div><div id="${prefix}MonthlyBox" style="${type==='monthly'?'':'display:none'}"><div class="field"><label>Quota mensile</label><input class="input" id="${prefix}MonthlyAmount" inputmode="decimal" value="${amount}" placeholder="Es. 120"></div><div class="field"><label>Durata abituale</label><select class="input" id="${prefix}MonthlyDuration">${([60,90].includes(dur)?[60,90]:[dur,60,90]).filter((v,i,a)=>a.indexOf(v)===i).map(v=>`<option value="${v}" ${dur===v?'selected':''}>${[60,90].includes(v)?durationLabel(v):durationLabel(v)+' · storico'}</option>`).join('')}</select></div><div class="note"><b>Mensile:</b> la quota non verrà richiesta a ogni lezione. Data, ora e modalità (DAD o presenza) vengono scelte ogni volta. La durata abituale serve solo come preselezione: puoi scegliere 1h o 1h30 e anche +2 a ogni appuntamento.</div></div>`;
}
...
function readBillingForm(prefix,base={}){
  ...
  const preferredRate=billingType==='lesson'?(document.getElementById(prefix+'PreferredRate')?.value||'individual'):'';
  return {
    billingType,
    preferredRate:['individual','regular','collective'].includes(preferredRate)?preferredRate:'',
...
function automaticWizardRate(){
  const nonMonthly=wizardNonMonthlyIds();
  if(!nonMonthly.length)return 'monthly';
  if(wizard.lessonType==='collective')return 'collective';
  if(wizard.lessonType==='individual'&&nonMonthly.length===1){
    const st=studentById(nonMonthly[0]);
    return st?.preferredRate==='regular'?'regular':'individual';
  }
  return 'individual';
}
function applyAutomaticWizardRate(){wizard.rateType=automaticWizardRate();return wizard.rateType}
...
function wizardDots(){
  ...
  else if(wizard.step===5)current=3;
...
function renderWizard(){
  if(wizard.step===0){
    ...
    applyAutomaticWizardRate();wizard.step=5;renderWizard();return;
...
function wizardNextStudents(){if(!wizard.studentIds.length){toast('Seleziona almeno un alunno');return}if(wizard.studentIds.length>MAX_COLLECTIVE_STUDENTS){toast(`Una collettiva può avere massimo ${MAX_COLLECTIVE_STUDENTS} alunni`);return}const preferredCollective=wizard.studentIds.length===1&&studentById(wizard.studentIds[0])?.preferredRate==='collective';wizard.lessonType=wizard.studentIds.length>1||preferredCollective?'collective':null;wizard.time='';wizard.selectedTimes=[];wizard.timeModes={};wizard.plannerStage='day';wizard.mode=null;wizard.rateType=wizard.lessonType==='collective'?automaticWizardRate():null;wizard.appointments=[];wizard.introStep='type';wizard.step=0;renderWizard()}
...
function finishWizardPlanning(){
  ...
  if(!wizard.quickDuplicate&&!wizard.recoveryOf)applyAutomaticWizardRate();
  wizard.step=5;wizard.plannerStage='day';renderWizard();
}
function wizardNextRate(){applyAutomaticWizardRate();wizard.step=5;renderWizard()}
...
function commitWizardLessons(mergeExisting=false){
  ...
  const created=[];let merged=0;const messageEntries=[];
  for(const a of analysis){
    if(!ensureMonthOpen(a.date.slice(0,7),'aggiungere una lezione'))return;
    if(a.mergeTarget){
      const addedIds=mergeStudentsIntoLesson(a.mergeTarget);
      if(addedIds===false)return;
      if(Array.isArray(addedIds)&&addedIds.length)messageEntries.push({lessonId:a.mergeTarget.id,studentIds:[...addedIds]});
      merged++;continue;
    }
    const l=buildLessonForOccurrence(a);data.lessons.push(l);created.push(l);messageEntries.push({lessonId:l.id,studentIds:[...wizard.studentIds]});lessonMonthlyIds(l).forEach(id=>ensureMonthlyPayment(id,l.date.slice(0,7),l.monthlyAmountsByStudent[id]));
  }
  ...
  postBookingEntries=messageEntries.filter(e=>{const l=data.lessons.find(x=>x.id===e.lessonId);return l&&l.status==='scheduled'&&!lessonHasEnded(l)&&e.studentIds?.length});
  if(postBookingEntries.length&&!wizard.recoveryOf)setTimeout(()=>openPostBookingMessages(),250);
}
function confirmWizard(){
  if(!(wizard.appointments||[]).length){toast('Nessun appuntamento da salvare');return}
  if(!wizard.quickDuplicate&&!wizard.recoveryOf)applyAutomaticWizardRate();
  const monthlyIds=wizardMonthlyIds(),nonMonthlyIds=wizardNonMonthlyIds(),price=nonMonthlyIds.length?rateAmount(wizard.rateType):0;if(nonMonthlyIds.length&&!price){toast('Tariffa automatica non valida');return}
...
function editSelectedStudentIds(){
  return [...document.querySelectorAll('input[name="editStudent"]:checked')].map(x=>x.value);
}
function automaticEditRate(l,ids,editType,monthlyIds){
  const billableIds=ids.filter(sid=>!monthlyIds.includes(sid));
  if(!billableIds.length)return 'monthly';
  if(editType==='collective')return 'collective';
  const sid=ids[0];
  if(l.studentIds.length===1&&l.studentIds[0]===sid&&['individual','regular'].includes(l.rateType))return l.rateType;
  const st=studentById(sid);
  return st?.preferredRate==='regular'?'regular':'individual';
}
function setEditLessonType(type){
  if(!['individual','collective'].includes(type))return;
  const input=document.getElementById('editLessonType');if(input)input.value=type;
  document.querySelectorAll('[data-edit-type]').forEach(btn=>btn.classList.toggle('selected',btn.dataset.editType===type));
  refreshEditLessonPricing();
}
function refreshEditLessonPricing(){
  const box=document.getElementById('editPricingPreview');if(!box)return;
  const id=box.dataset.lessonId,l=data.lessons.find(x=>x.id===id);if(!l)return;
  const ids=editSelectedStudentIds(),editType=document.getElementById('editLessonType')?.value||'individual';
  if(!ids.length){box.innerHTML='<b>Seleziona almeno un alunno.</b>';return}
  if(editType==='individual'&&ids.length!==1){box.innerHTML='<b>Individuale:</b> lascia selezionato un solo alunno.';return}
  const monthlyIds=ids.filter(sid=>lessonMonthlyIds(l).includes(sid)||isMonthlyStudent(sid));
  const billableIds=ids.filter(sid=>!monthlyIds.includes(sid));
  const rate=automaticEditRate(l,ids,editType,monthlyIds),units=Number(document.getElementById('editLessonUnits')?.value)||1;
  if(!billableIds.length){
    box.innerHTML=`<b>${editType==='collective'?'Collettiva':'Individuale'} · piano mensile</b><br>Nessun costo a singola lezione. Il mese resta conteggiato con la quota mensile.`;
    return;
  }
  const price=rateAmount(rate),perStudent=price*units,total=perStudent*billableIds.length;
  if(editType==='collective'){
    box.innerHTML=`<b>Collettiva · ${fmtEuro(price)} per alunno / lezione</b><br>${billableIds.length} ${billableIds.length===1?'alunno non mensile':'alunni non mensili'} × ${units} ${units===1?'lezione':'lezioni'} = <b>${fmtEuro(total)}</b>${monthlyIds.length?` · ${monthlyIds.length} mensile/i senza costo a lezione`:''}.`;
  }else{
    box.innerHTML=`<b>${rateLabel(rate)} · ${fmtEuro(price)} per lezione</b><br>${units} ${units===1?'lezione':'lezioni'} = <b>${fmtEuro(total)}</b>.`;
  }
}
function openLessonEdit(id){
  const l=data.lessons.find(x=>x.id===id);if(!l)return;
  const times=allowedLessonTimes(),base=lessonBaseMinutes(l),units=lessonUnitCount(l),currentType=lessonIsCollective(l)?'collective':'individual';
  const baseOpts=([60,90].includes(base)?[60,90]:[base,60,90]).filter((v,i,a)=>a.indexOf(v)===i);
  const people=activeStudents().concat(archivedStudents().filter(st=>l.studentIds.includes(st.id))).filter((st,i,a)=>a.findIndex(x=>x.id===st.id)===i).sort((a,b)=>a.name.localeCompare(b.name));
  showModal(`<h2>Modifica lezione</h2><p class="sub">Puoi cambiare completamente la lezione: tipologia, alunni, data, ora, durata e modalità.</p><div class="field"><label>Tipo lezione</label><input type="hidden" id="editLessonType" value="${currentType}"><div class="choices"><button type="button" class="choice ${currentType==='individual'?'selected':''}" data-edit-type="individual" onclick="setEditLessonType('individual')"><b>👤 Individuale</b><small>1 solo alunno</small></button><button type="button" class="choice ${currentType==='collective'?'selected':''}" data-edit-type="collective" onclick="setEditLessonType('collective')"><b>👥 Collettiva</b><small>1 o più alunni · €10 per i non mensili</small></button></div></div><div class="field"><label>Alunni</label>${people.map(st=>`<label class="studentpick"><input type="checkbox" name="editStudent" value="${st.id}" ${l.studentIds.includes(st.id)?'checked':''} onchange="refreshEditLessonPricing()"><span>${esc(st.name)}${studentBillingType(st)==='monthly'?' · Mensile':''}${st.active===false?' · eliminato':''}</span></label>`).join('')}</div><div class="row"><div class="field"><label>Data</label><input class="input" id="editDate" type="date" value="${l.date}"></div><div class="field"><label>Ora inizio</label><select class="input" id="editTime">${times.map(t=>`<option ${t===l.time?'selected':''}>${t}</option>`).join('')}</select></div></div><div class="row"><div class="field"><label>Durata singola</label><select class="input" id="editBaseDuration">${baseOpts.map(v=>`<option value="${v}" ${v===base?'selected':''}>${[60,90].includes(v)?durationLabel(v):durationLabel(v)+' · storico'}</option>`).join('')}</select></div><div class="field"><label>Lezioni consecutive</label><select class="input" id="editLessonUnits" onchange="refreshEditLessonPricing()"><option value="1" ${units===1?'selected':''}>+1</option><option value="2" ${units===2?'selected':''}>+2</option></select></div></div><div class="field"><label>Modalità</label><select class="input" id="editMode"><option value="presence" ${l.mode==='presence'?'selected':''}>Presenza</option><option value="dad" ${l.mode==='dad'?'selected':''}>DAD</option></select></div><div class="note price-note" id="editPricingPreview" data-lesson-id="${id}"></div><div class="note"><b>Tariffa automatica:</b> Individuale usa la tariffa salvata dell’alunno (€20 o abituale €15); Collettiva usa €10 per ogni alunno non mensile. Gli alunni mensili non aggiungono costi alla singola lezione.</div><button class="cta" onclick="saveLessonEdit('${id}')">Salva modifiche</button>`);
  setTimeout(refreshEditLessonPricing,0);
}
function saveLessonEdit(id){
  ...
  save('Modifica lezione completa');modalStack.pop();openLesson(id,true);toast(editType==='collective'?'Lezione aggiornata come collettiva':'Lezione aggiornata come individuale');
}
function buildMessage(l,s){
  const kind=l.mode==='dad'?'in modalità DAD':'in presenza',group=lessonIsCollective(l)?' La lezione sarà collettiva.':'',units=lessonUnitCount(l)>1?` Sono ${lessonUnitCount(l)} lezioni consecutive da ${durationLabel(lessonBaseMinutes(l))} (${durationLabel(l.duration)} totali).`:'';
  return `Ciao ${s.name.split(' ')[0]}, confermo la lezione di matematica per ${fmtDate(l.date)} dalle ${l.time} alle ${endTimeFromStart(l.time,l.duration)}, ${kind}.${group}${units}`;
}
function weekStartKey(k){
  const d=parseLocalDate(k),delta=(d.getDay()+6)%7;d.setDate(d.getDate()-delta);return dateKey(d);
}
function weekEndKey(startKey){const d=parseLocalDate(startKey);d.setDate(d.getDate()+6);return dateKey(d)}
function fmtWeekRange(startKey){
  const a=parseLocalDate(startKey),b=parseLocalDate(weekEndKey(startKey));
  if(a.getMonth()===b.getMonth())return `${a.getDate()}–${b.getDate()} ${months[a.getMonth()].toLowerCase()}`;
  return `${a.getDate()} ${months[a.getMonth()].toLowerCase()} – ${b.getDate()} ${months[b.getMonth()].toLowerCase()}`;
}
function postBookingLessonsForStudent(sid){
  const seen=new Set(),out=[];
  postBookingEntries.forEach(e=>{if(!e.studentIds?.includes(sid)||seen.has(e.lessonId))return;const l=data.lessons.find(x=>x.id===e.lessonId);if(l&&l.status==='scheduled'&&!lessonHasEnded(l)){seen.add(l.id);out.push(l)}});
  return out.sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time));
}
function weeklyBookingGroupsForStudent(sid){
  const groups={};postBookingLessonsForStudent(sid).forEach(l=>{const wk=weekStartKey(l.date);(groups[wk]||(groups[wk]=[])).push(l)});return groups;
}
function weeklyLessonLine(l){
  const mode=l.mode==='dad'?'DAD':'Presenza',group=lessonIsCollective(l)?' · collettiva':'',units=lessonUnitCount(l)>1?` · ${lessonUnitsLabel(l)}`:'';
  return `• ${fmtDate(l.date)} · ${l.time}–${endTimeFromStart(l.time,l.duration)} · ${mode}${group}${units}`;
}
function buildWeeklyMessage(s,lessons){
  const sorted=[...lessons].sort((a,b)=>(a.date+a.time).localeCompare(b.date+b.time));
  return `Ciao ${s.name.split(' ')[0]}, ti confermo le lezioni di matematica della settimana:\n\n${sorted.map(weeklyLessonLine).join('\n')}\n\nA presto!`;
}
function sendWeeklyWhatsApp(sid,weekKey,target='student'){
  const s=studentById(sid);if(!s)return;const lessons=weeklyBookingGroupsForStudent(sid)[weekKey]||[];if(lessons.length<2){toast('Servono almeno 2 appuntamenti nella stessa settimana');return}
  const phone=target==='parent'?(s.parentPhone||s.phone):s.phone;if(!phone){toast('Numero non inserito');return}window.location.href=waUrl(phone,buildWeeklyMessage(s,lessons));
}
function openPostBookingSingles(sid){
  const s=studentById(sid);if(!s)return;const lessons=postBookingLessonsForStudent(sid);
  replaceModal(`<h2>Messaggi singoli</h2><p class="sub">${esc(s.name)} · ${lessons.length} ${lessons.length===1?'appuntamento':'appuntamenti'} appena prenotati.</p>${lessons.map(l=>`<div class="card"><b>${fmtDate(l.date)} · ${lessonTimeRange(l)}</b><p style="color:var(--muted);font-size:12px">${esc(buildMessage(l,s))}</p><div class="sendgrid"><button class="smallbtn wa" onclick="sendWhatsApp('${l.id}','${sid}','student')">WhatsApp alunno</button>${s.parentPhone?`<button class="smallbtn wa" onclick="sendWhatsApp('${l.id}','${sid}','parent')">WhatsApp genitore</button>`:'<button class="smallbtn" disabled>Nessun genitore</button>'}</div></div>`).join('')}<button class="cta secondary" onclick="openPostBookingMessages(true)">← Riepilogo invii</button>`);
}
function openPostBookingMessages(replace=false){
  const studentIds=[...new Set(postBookingEntries.flatMap(e=>e.studentIds||[]))].filter(id=>studentById(id));
  if(!studentIds.length){closeModal();return}
  const cards=studentIds.map(sid=>{const s=studentById(sid),lessons=postBookingLessonsForStudent(sid),groups=weeklyBookingGroupsForStudent(sid),weekly=Object.entries(groups).filter(([,ls])=>ls.length>=2);
    return `<div class="card"><div class="summary-head"><div><h3 style="margin:0 0 4px">${esc(s.name)}</h3><p style="margin:0;color:var(--muted);font-size:12px">${lessons.length} ${lessons.length===1?'appuntamento':'appuntamenti'} appena prenotati</p></div></div>${weekly.map(([wk,ls])=>`<div class="note price-note" style="margin-top:10px"><b>📅 Settimana ${fmtWeekRange(wk)}</b><br>${ls.map(l=>`${fmtDate(l.date)} · ${l.time}`).join(' · ')}<div class="sendgrid"><button class="smallbtn wa" onclick="sendWeeklyWhatsApp('${sid}','${wk}','student')">Unico WhatsApp</button>${s.parentPhone?`<button class="smallbtn wa" onclick="sendWeeklyWhatsApp('${sid}','${wk}','parent')">Al genitore</button>`:''}</div></div>`).join('')}<button class="cta secondary compact-cta" onclick="openPostBookingSingles('${sid}')">Messaggi singoli (${lessons.length})</button></div>`}).join('');
  const html=`<h2>Conferme WhatsApp</h2><p class="sub">Puoi inviare ogni appuntamento singolarmente oppure, quando lo stesso alunno ha più lezioni nella stessa settimana, usare un unico riepilogo settimanale.</p>${cards}<button class="cta secondary" onclick="closeModal()">Chiudi</button>`;
